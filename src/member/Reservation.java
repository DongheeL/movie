...
public class Reservation implements Serializable{
	...
	void printMovie() {
		if(mList.size() != 0) {
			for(int i=0; i<mList.size(); i++) {
				System.out.println((char)(65+i)+". "+"제     목 : " + mList.get(i).getTitle()  
						+ "\n     감      독 : " + mList.get(i).getDirector()  
						+ "\n     평      점 : " + mList.get(i).getScore()  
						+ "\n     출시 연도 : " + mList.get(i).getYear());
				System.out.println("------------------");
			}
		}
		else {
	...
	public String selMovie() {
		//System.out.print("선택하실 영화제목을 작성해 주세요 ");
	...
	public String mvTime(String movie) {
		...
		//System.out.println("상영하실 영화를 선택하세요. ");
		String showTimes = null;
		if(mList.get(0).getTitle().equals(movie)) {
			showTimes = "영화의 상영시작 시간은 : 1.[10:00] , 2.[14:00] , 3.[18:00] 입니다.";
		}
		else if(mList.get(1).getTitle().equals(movie)) {
			showTimes = "영화의 상영시작 시간은 : 1.[09:00] , 2.[12:00] , 3.[15:00] 입니다.";
		}
		else if(mList.get(2).getTitle().equals(movie)) {
			showTimes = "영화의 상영시작 시간은 : 1.[12:00] , 2.[17:00] , 3.[22:00] 입니다.";
		}
		else {
...
//		}
	...
	String selMovieTime(String movie,String showTimes) {
		...
		//System.out.print("상영시간을 선택하여 주세요. ");
		int time = sc.nextInt();
		if(time>0 && time<4) {
	...
	String seat(MemberController memc) {
		...
		// 좌석 선택
		System.out.print("원하는 좌석 선택해주세요.ex)A열 4행: A4 > ");
